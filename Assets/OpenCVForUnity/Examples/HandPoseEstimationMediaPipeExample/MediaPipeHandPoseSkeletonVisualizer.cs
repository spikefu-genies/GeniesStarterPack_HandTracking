...
namespace OpenCVForUnityExample.DnnModel
{
    ...
    public class MediaPipeHandPoseSkeletonVisualizer : MonoBehaviour
    {
        ...
        public void UpdatePoseAndNormal(Vector3[] landmarks_world, Vector3 normal)
        {
            ...
            {
                skeletons[20].Line.SetPosition(0, new Vector3(
                    landmarks_world[0].x * skeletonScale * 1 + skeletonX,
                    landmarks_world[0].y * skeletonScale * -1 + skeletonY,
                    landmarks_world[0].z * skeletonScale * 1 + skeletonZ
                    ));
                skeletons[20].Line.SetPosition(1, new Vector3(
                    (landmarks_world[0].x + normal.x) * skeletonScale * 1 + skeletonX,
                    (landmarks_world[0].y + normal.y) * skeletonScale * -1 + skeletonY,
                    (landmarks_world[0].z + normal.z) * skeletonScale * 1 + skeletonZ
                     ));
            }
        }

        public void UpdatePoseAndNormal(Mat result, Vector3 normal)
        {
            if (result.empty() || result.rows() < 132)
                return;
            ...
            UpdatePoseAndNormal(landmarks_world_buffer, normal);
        }
